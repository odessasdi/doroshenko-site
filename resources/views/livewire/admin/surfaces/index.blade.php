<div class="space-y-6">
    <div class="flex items-start justify-between gap-4">
        <div>
            <h1 class="text-2xl font-semibold text-zinc-900">Основи</h1>
            <p class="mt-1 text-sm text-zinc-600">Керування списком основ.</p>
        </div>
        <button
            type="button"
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-[1.5fr_1fr]">
            ...
            <table class="min-w-full divide-y divide-zinc-200 text-sm">
                <thead class="bg-zinc-50 text-left text-xs font-semibold uppercase tracking-wide text-zinc-500">
                    <tr>
                        ...
                        <th class="px-4 py-3">Назва (EN)</th>
                        <th class="px-4 py-3">Назва (DE)</th>
                        <th class="px-4 py-3">Назва (UA)</th>
                ...
                <tbody class="divide-y divide-zinc-200">
                    @forelse ($surfaces as $surface)
                        <tr class="text-zinc-800">
                            <td class="px-4 py-3">{{ $surface->id }}</td>
                            <td class="px-4 py-3">{{ $surface->name_en }}</td>
                            <td class="px-4 py-3">{{ $surface->name_de }}</td>
                            <td class="px-4 py-3">{{ $surface->name_ua }}</td>
                            <td class="px-4 py-3 text-right">
                                <div class="flex justify-end gap-2">
                                    ...
                                    <button
                                        ...
                                        wire:click="delete({{ $surface->id }})"
                                        onclick="if (!confirm('Видалити цю основу?')) { event.stopImmediatePropagation(); event.preventDefault(); }"
                                    >
                                        Видалити
                                    </button>
                                </div>
                    ...
                    @empty
                        <tr>
                            <td class="px-4 py-6 text-center text-zinc-500" colspan="5">Поки що основ немає.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div class="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm">
            <h2 class="text-lg font-semibold text-zinc-900">
                {{ $editingId ? 'Редагувати основу' : 'Нова основа' }}
            </h2>
            <div class="mt-4 grid grid-cols-1 gap-4">
                <div>
                    <label class="text-sm font-medium text-zinc-700" for="name_en">Назва (EN)</label>
                    <input
                        id="name_en"
                        type="text"
                ...
                <div>
                    <label class="text-sm font-medium text-zinc-700" for="name_de">Назва (DE)</label>
                    <input
                        id="name_de"
                        type="text"
                        class="mt-1 w-full rounded-lg border-zinc-300 focus:border-zinc-900 focus:ring-zinc-900"
                ...
                <div>
                    <label class="text-sm font-medium text-zinc-700" for="name_ua">Назва (UA)</label>
                    <input
                        id="name_ua"
                        type="text"
                        class="mt-1 w-full rounded-lg border-zinc-300 focus:border-zinc-900 focus:ring-zinc-900"
